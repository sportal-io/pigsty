...
Monitor.prototype.stop = function() {
  ...
  if (self.bookmark_sync)
    clearInterval(self.bookmark_sync);
...
Monitor.prototype._write_bookmark = function(now) {
  ...
  var write = function() {
    ...
    if (now) {
      ...
      debug("Sync writing bookmark: ", bookmark, "to", tmp);
      self.writer = false;
      
      try {
        fs.writeFileSync(bookmark, tmp);
      } catch (err) {
        console.error("[!] Unable to write bookmark file: ", err);
        console.error("[!] Do you have permissions to write to ", bookmark, "?");
        process.exit(1);
      }
