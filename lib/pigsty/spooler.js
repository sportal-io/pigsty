...
Spooler.prototype._send_to_plugins = function(event) {
  ...
  // if (false && pause_required && !self.paused) {
    // self.file.pause();
    // var check = function() {
      // var pause_required = false;
      // _.each(self.output_plugins, function(plugin) {
        // if (plugin.pending > 1000) {
          // console.log("paused... too much data:", plugin.pending);
          // pause_required = true; 
        // }
      // });

      // if (!pause_required) {
        // console.log("XXX: UNPAUSING")
        // self.file.unpause();
    ...
    // self.paused = setInterval(check, 5000);
  // };
  ...
  _.each(Object.keys(self.options.output), function(key) {
    ...
    var setup = function(config) {
      ...
      try {
        ...
        console.error("[X] There was a problem installing the output plugin: " + plugin_name);
        console.error("[X] Error was:", err);
        console.error("[X] Do you have the plugin installed? Try doing: npm install " + plugin_name);
